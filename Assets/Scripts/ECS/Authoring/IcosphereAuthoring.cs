...
public class IcosphereAuthoring : MonoBehaviour
{
    ...
    public class Baker : Baker<IcosphereAuthoring>
    {
        public override void Bake(IcosphereAuthoring authoring)
        {
            ...
            AddComponent(entity, new IcosphereDefinition
            {
                Radius = (authoring.transform).localScale.x / 2f,
                TesselationLevel = authoring.TesselationLevel
            });
        }
    }
}

// Component to hold the definition data
public struct IcosphereDefinition : IComponentData
{
    public float Radius;
...
public partial class IcosphereCreateSystem : SystemBase
{
    protected override void OnCreate()
    {
        // Make sure the system runs only once during initialization
        RequireForUpdate<IcosphereDefinition>();
    }

    protected override void OnUpdate()
    {
        // Process all entities with IcosphereDefinition but without IcosphereComponent
        EntityQuery query = SystemAPI.QueryBuilder()
            .WithAll<IcosphereDefinition>()
    ...
    private BlobAssetReference<IcosphereMapBlob> CreateIcosphereBlob(float radius, int tesselationLevel)
    {
        ...
        for (int i = 0; i < triangles.Length; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                // Calculate the actual neighbors
