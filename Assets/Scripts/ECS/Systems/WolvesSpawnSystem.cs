...
public partial struct WolvesSpawnSystem : ISystem
{
    [BurstCompile]
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        var wolvesCountVar = config.BlobReference.Value.World.WolfSpawn.Count;
        var wolvesCount = MathExtentions.GetRandomVariation(ref randomData.ValueRW.Random, wolvesCountVar);
        ...
        for (int i = 0; i < wolvesCount; i++)
        {
            ...
            WolfsFactory.SpawnWolf(newPosition, newRotation, commandBuffer, randomData, config.BlobReference.Value);
        }
